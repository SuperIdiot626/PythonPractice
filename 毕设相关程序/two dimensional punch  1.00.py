...
while 1:
    ...
    if wait_shot==0:   #计算下一时刻位置
        ...
        tracks.append((ball_x,ball_y))                  #添加一个路径点
        if len(tracks)>=2:
    ...
    distance=distance**0.5      #球心距离用于碰撞判定

    if distance<=2*radius and punched==0:
        if delta_x>=0:
    ...
    if ball_x+time_pass*velocity[0]<=radius or ball_x+time_pass*velocity[0]>=width-radius:
        velocity[0]=-velocity[0]
    if ball_y+time_pass*velocity[1]<=radius or ball_y+time_pass*velocity[1]>=height-radius:
