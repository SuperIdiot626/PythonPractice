...
class balls(object):
    ...
    def acce_compute(self):                                                     #加速度计算，包括压力项与粘性项
        ...
        for i in self.nearest_dots:
            i=ball_list[i]
            d_pos=minus(self.pos,i.pos)                                         #位置差
            d_vel=minus(self.vel,i.vel)                                         #速度差
    ...
    def surface_tension(self):
        ...
        if len(self.nearest_dots)  == 0:
            ...
            if magenititude_simple(gradient)>=layer_thick:                      #用于判断粒子是否处于表层
                ...
                surface_force=plus(surface_force,times(gradient,-5*curvature))  #加速度相加即可
            else:
    ...
    def compute_rho(self):                          #密度更新
        self.rho=0                                  #每次使用将这一值更新
        for i in self.nearest_dots:
            self.rho+=norma_kenel(i.pos,self.pos,smooth_length)
...
def initialize():
    for i in ball_list:
        dy=(i.pos[1]-ball_list[0].pos[1])/100                                  #以最后一个粒子的高度为参照（粒子生成是从上至下的）
