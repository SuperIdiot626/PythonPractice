...
class balls(object):
    ...
    def acceleratecompute(self):
        self.acce=plus(times(self.vel,miu),[0,1000])

    def punch(self):                                #边界碰撞检测
        if self.pos[1]+self.vel[1]*time_pass<=self.radius:                      #若下一时刻粒子超出 上 边界
            self.pos[1]=self.radius                
            self.vel[1]=-self.vel[1]
        if self.pos[1]+self.vel[1]*time_pass>=height-self.radius:               #若下一时刻粒子超出 下 边界
            self.pos[1]=height-self.radius   
            self.vel[1]=-self.vel[1]
        if self.pos[0]+self.vel[0]*time_pass<=self.radius:                      #若下一时刻粒子超出 左 边界
            self.pos[0]=self.radius          
            self.vel[0]=-self.vel[0]
        if self.pos[0]+self.vel[0]*time_pass>=width-self.radius:                #若下一时刻粒子超出 右 边界
...
def check_ball():                                   #检测并显示粒子
    ...
    pygame.draw.circle(screen,(255,255,255),converse_int(ball_list[check_id].pos),10,2)                 #显示目前选中粒子的位置
...
while 1:
    ...
    for i in balls_in_grid_list[grid_list.index(show_grid)]:                                            #显示鼠标所在格子的粒子
        pygame.draw.circle(screen,(255,0,0),converse_int(ball_list[i].pos),15,3)
