...
class balls(object):
    ...
    def acce_compute(self):
        ...
        for i in self.near_dots:
            ...
            if distance<=0.1*smooth_length:         #粒子间距过近为斥力 
                a=times(distance_vec, 10000*distance**(-2))
                self.acce=plus(self.acce,a)        
            if distance<=smooth_length:             #粒子间距较远为引力 
                a=times(distance_vec, 100*distance**(-2))
                self.acce=minus(self.acce,a)        


    def acce_compute_new(self):
        ...
        if len(self.nearest_dots)  == 0:
            ...
            for i in self.nearest_dots:
                ...
                gradient=plus(gradient,nabla_kenel_result)
                Csi += norma_kenel(self.pos,ball_list[i].pos,smooth_length)
            gradient=divide(gradient,len(self.nearest_dots))                            #梯度要最后除以粒子数量
            Csi/=len(self.nearest_dots)                                                 #计算该粒子的Csi
            Csi-=1
            if magenititude_simple(gradient)>=layer_thick:          #用于判断粒子是否处于表层
                self.color=(255,0,0)                                #对于处在表层的粒子，颜色变红作为标记
                ...
                self.acce=plus(self.acce,times(gradient,-20*curvature))                      #加速度相加即可
            else:
    ...
    def new_pos_vel(self):
        ...
        self.acce_compute_new()                                                 #不论哪种方法，先计算加速度
...
def check_ball():                                   #检测并显示粒子
    ...
    for i in ball_list[check_id].near_dots:
        pygame.draw.circle(screen,(255,255,255),converse_int(ball_list[i].pos),6,2)                     #白色圈出附近粒子
    for i in ball_list[check_id].nearest_dots:
        pygame.draw.circle(screen,(0,0,255),converse_int(ball_list[i].pos),6,2)                         #蓝色圈出影响域圆内粒子    
    ...
    for i in balls_in_grid_list[grid_list.index(show_grid)]:                                            #显示鼠标所在格子的粒子
        pygame.draw.circle(screen,(0,255,0),converse_int(ball_list[i].pos),6,3)
