...
def datacollect(dir):                                   #function to collect data
    ...
    wholeTxt.reverse()
    wholeTxt=wholeTxt[0:1001:1000]                      #only need the last 50 lines 
    #print(wholeTxt[0])
    ...
    for i in wholeTxt[0][-2:]:
        statistical_result.write(i+'   ') 
    lift_coe=float(wholeTxt[0][-2])
    drag_coe=float(wholeTxt[0][-1])
    statistical_result.write('L/D=') 
    try:
        statistical_result.write('%1.7e'%(lift_coe/drag_coe)) 
    except ZeroDivisionError:
        statistical_result.write('divided by zero') 
    statistical_result.write('\n')

    statistical_result.write("    last 1000 steps       "+wholeTxt[1][0]+'      ') 
    for i in wholeTxt[1][-2:]:
        statistical_result.write(i+'   ') 
    lift_coe=float(wholeTxt[1][-2])
    drag_coe=float(wholeTxt[1][-1])
    statistical_result.write('L/D=') 
    try:
        statistical_result.write('%1.7e'%(lift_coe/drag_coe)) 
    except ZeroDivisionError:
    ...
    statistical_result.write("L/D=") 
    try:
        statistical_result.write('%1.7e'%(average[-2]/average[-1])) 
    except ZeroDivisionError:
        statistical_result.write('divided by zero')
    statistical_result.write('\n')'''
