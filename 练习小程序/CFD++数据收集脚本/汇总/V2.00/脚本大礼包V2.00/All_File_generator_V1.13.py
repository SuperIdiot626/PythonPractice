...
wirte_mcfd_inp=0            # 需要写入哪些文件就将其值设为1，否则设为零
wirte_job1_cfd=1
wirte_infout_inp=0
wirte_tecout_inp=0
    ...
    for i in text:
        ...
        if i[-2:]=='-\n':
            ...
            if num_ends==2:
                ...
                beginline=endline
                endline=num_lines
                filewriteParts(file,text[beginline:endline-13] )
                file.write('pref %e\n'%pref)
                file.write('rref %e\n'%rref)
                file.write('uref %e\n'%uref)
                filewriteParts(file,text[endline-10:endline-4] )
                file.write('alpha '+states[-1]+'\n')
                filewriteParts(file,text[endline-3:endline] )
    file.close()
...
#v1.13 2022年9月15日21:15:02
#更新内容：
#1.对infout1f.inp的修改可以改多个entry了，会根据states中的配置改变其来流参考以及攻角，其他不变
#toDo：
#暂无