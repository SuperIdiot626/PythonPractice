...
class MY_GUI():
    ...
    def set_init_window(self):
        ...
        self.init_window_name.geometry('500x300+100+100')
        ...
        self.pbefore_label = Label(self.init_window_name, text="请输入波前压强p0")
        self.pbefore_label.grid(row=1, column=0)                                #row和column分别为第几行第几列
        ...
        self.pafter_label = Label(self.init_window_name, text="请输入波后压强p1")
        self.pafter_label.grid(row=2, column=0)
        ...
        self.result_data_label = Label(self.init_window_name, text="马赫数输出")
        self.result_data_label.grid(row=3, column=0)
        self.result_data_Text = Text(self.init_window_name, width=20, height=1)  #处理结果展示
        self.result_data_Text.grid(row=3, column=1, rowspan=1, columnspan=1)

        self.log_label = Label(self.init_window_name, text="日志")
        self.log_label.grid(row=4, column=0)
        self.log_data_Text = Text(self.init_window_name, width=66, height=9)  # 日志框
        self.log_data_Text.grid(row=5, column=13, columnspan=5)
        
        
        #按钮
        self.calculate = Button(self.init_window_name, text="计算", bg="lightblue", width=10,command=self.calculate)  # 调用内部方法  加()为直接调用
        self.calculate.grid(row=1, column=5)
