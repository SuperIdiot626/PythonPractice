...
for dir in secondarydirlsit:                        #遍历文件夹以及文件
    ...
    statistical_result.write('    当前步数'+wholeTxt[0][0]+'     ') #写入步数
    for i in wholeTxt[0][1:]:
        statistical_result.write(i+'  ') 
    lift_coe=float(wholeTxt[0][-2])
    drag_coe=float(wholeTxt[0][-1])
    statistical_result.write('运算结果为') 
    try:
        statistical_result.write(str(lift_coe/drag_coe)[:10]) 
    except ZeroDivisionError:
        statistical_result.write('除数为零，运算结果无效')  

    statistical_result.write('\n')      
    statistical_result.write('前一千步步数'+wholeTxt[1][0]+'     ') #写入步数
    for i in wholeTxt[1][1:]:
        statistical_result.write(i+'  ') 
