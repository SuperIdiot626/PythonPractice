...
import openpyxl,os
import time
excelFileName=time.strftime(("%Y-%m-%d_%H.%M.%S"),time.localtime())
excelFileName='data_'+excelFileName+'.xlsx'
...
B1_tou      =zeros(((2*n1  )*(n0  ),3))
B2_tou      =zeros(((2*n1  )*(n0  ),3))
...
def GCGS(   n0=None,
    ...
    for m in range(0,n0-1):
        ...
        ####################二锥####################
        #bug,已经修改了m的取值
        for p in range(n1,2*n1-1):
            T1_up   [2*(n1-1)* m   +p-1]=(  #修改了n1的取值  bug
                + B1[2* n1   *(m+1)+p+1]
                - B1[2* n1   * m   +p  ]
                )
            T2_up   [2*(n1-1)* m   +p-1]=(
            ...
            B1_tou[2* n1   * m   +p  ]=B1[2* n1   * m   +p  ] + N_B1_danwei[2*(n1-1)* m   +p-1]*(Average_B1[2*(n1-1)* m   +p-1] - B1[2* n1   * m   +p  ])
            B1_tou[2* n1   *(m+1)+p  ]=B1[2* n1   *(m+1)+p  ] + N_B1_danwei[2*(n1-1)* m   +p-1]*(Average_B1[2*(n1-1)* m   +p-1] - B1[2* n1   *(m+1)+p  ])
            B1_tou[2* n1   *(m+1)+p+1]=B1[2* n1   *(m+1)+p+1] + N_B1_danwei[2*(n1-1)* m   +p-1]*(Average_B1[2*(n1-1)* m   +p-1] - B1[2* n1   *(m+1)+p+1])
            B1_tou[2* n1   * m   +p+1]=B1[2* n1   * m   +p+1] + N_B1_danwei[2*(n1-1)* m   +p-1]*(Average_B1[2*(n1-1)* m   +p-1] - B1[2* n1   * m   +p+1])
            ...
            A_up[2*(n1-1)* m   +p-1]=(0.5*(B2_myx[2* n1   *(m+1)+p+1][0] - B2_myx[2* n1   * m   +p  ][0])
            ...
            CA_up[2*(n1-1)* m   +p-1]=F_up [2*(n1-1)* m   +p-1][0]*N_B1_danwei[2*(n1-1)* m   +p-1][0] / (0.5*midu*(V[0][0]**2+V[0][1]**2+V[0][2]**2)*S_can)
            CN_up[2*(n1-1)* m   +p-1]=F_up [2*(n1-1)* m   +p-1][2]*N_B1_danwei[2*(n1-1)* m   +p-1][2] / (0.5*midu*(V[0][0]**2+V[0][1]**2+V[0][2]**2)*S_can)
            CZ_up[2*(n1-1)* m   +p-1]=F_up [2*(n1-1)* m   +p-1][1]*N_B1_danwei[2*(n1-1)* m   +p-1][1] / (0.5*midu*(V[0][0]**2+V[0][1]**2+V[0][2]**2)*S_can)
    ...
    for m in range(0,n0 - 1):
        ...
        for p in range(0,n1 - 1):
            T1_down [2*(n1-1)* m   +p  ]=(      #bug 修改了n1取值
                + B2[2* n1   *(m+1)+p+1]
                - B2[2* n1   * m   +p  ]
                )
            T2_down [2*(n1-1)* m   +p  ]=(
            ...
            N_B2_fanshu[2*(n1-1)* m   +p  ]=linalg.norm(N_B2[2*(n1-1)* m   +p  ])
            ...
            #将四个角点投影到面元平面上
            B2_tou[2* n1   * m   +p  ]=B2[2* n1   * m   +p  ] + N_B2_danwei[2*(n1-1)* m   +p  ]*(Average_B2[2*(n1-1)* m   +p  ] - B2[2* n1   * m   +p  ])
            B2_tou[2* n1   *(m+1)+p  ]=B2[2* n1   *(m+1)+p  ] + N_B2_danwei[2*(n1-1)* m   +p  ]*(Average_B2[2*(n1-1)* m   +p  ] - B2[2* n1   *(m+1)+p  ])
            B2_tou[2* n1   *(m+1)+p+1]=B2[2* n1   *(m+1)+p+1] + N_B2_danwei[2*(n1-1)* m   +p  ]*(Average_B2[2*(n1-1)* m   +p  ] - B2[2* n1   *(m+1)+p+1])
            B2_tou[2* n1   * m   +p+1]=B2[2* n1   * m   +p+1] + N_B2_danwei[2*(n1-1)* m   +p  ]*(Average_B2[2*(n1-1)* m   +p  ] - B2[2* n1   * m   +p+1])
            
            
            #建立面元坐标系:原点位于X_bar、Y_bar、Z_bar，x轴平行于t1,y轴平行于m，z轴平行于n
            T1_down_fanshu[2*(n1-1)* m   +p  ]=linalg.norm(T1_down[2*(n1-1)* m   +p  ])
            ...
            B2_myx[2* n1   * m   +p  ][1]=(m_danwei_down[2*(n1-1)* m   +p  ][0]*(B2_tou[2* n1   * m   +p  ][0] - Average_B2[2*(n1-1)* m   +p  ][0])
                                         + m_danwei_down[2*(n1-1)* m   +p  ][1]*(B2_tou[2* n1   * m   +p  ][1] - Average_B2[2*(n1-1)* m   +p  ][1])
            ...
            zhuangjijiao_down [2*(n1-1)* m   +p  ]=pi/2-arccos(-(dot(N_B2_danwei[2*(n1-1)* m   +p  ],V[0])))/ linalg.norm(V[0])
            if zhuangjijiao_down[2*(n1-1)* m   +p  ] >= 0:
                Cp_down[2*(n1-1)* m   +p  ]=Newton1(zhuangjijiao_down[2*(n1-1)* m   +p  ],V[0][0],V[0][1],V[0][2],Ma)
            if zhuangjijiao_down[2*(n1-1)* m   +p  ] < 0:
                Cp_down[2*(n1-1)* m   +p  ]=Qie2   (zhuangjijiao_down[2*(n1-1)* m   +p  ],V[0][0],V[0][1],V[0][2],Ma)
            #求作用力
        ...
        ####################二锥####################
        for p in range(n1,2*n1-1):
            T1_down  [2*(n1-1)* m   +p-1]=(     #bug 修改了n1取值
                +  B2[2* n1   *(m+1)+p+1]
                -  B2[2* n1   * m   +p  ]
                )
            ...
            N_B2_fanshu[2*(n1-1)* m   +p-1]=linalg.norm(N_B2[2*(n1-1)* m   +p-1])
            ...
            #将四个角点投影到面元平面上
            B2_tou[2* n1   * m   +p  ]=B2[2* n1   * m   +p  ] + N_B2_danwei[2*(n1-1)* m   +p-1]*(Average_B2[2*(n1-1)* m   +p-1] - B2[2* n1   * m   +p  ])
            B2_tou[2* n1   *(m+1)+p  ]=B2[2* n1   *(m+1)+p  ] + N_B2_danwei[2*(n1-1)* m   +p-1]*(Average_B2[2*(n1-1)* m   +p-1] - B2[2* n1   *(m+1)+p  ])
            B2_tou[2* n1   *(m+1)+p+1]=B2[2* n1   *(m+1)+p+1] + N_B2_danwei[2*(n1-1)* m   +p-1]*(Average_B2[2*(n1-1)* m   +p-1] - B2[2* n1   *(m+1)+p+1])
            B2_tou[2* n1   * m   +p+1]=B2[2* n1   * m   +p+1] + N_B2_danwei[2*(n1-1)* m   +p-1]*(Average_B2[2*(n1-1)* m   +p-1] - B2[2* n1   * m   +p+1])
            ...
            #求面元的面积△A
            A_down[2*(n1-1)* m   +p-1]=(0.5*(B2_myx[2* n1   *(m+1)+p+1][0] - B2_myx[2* n1   * m   +p  ][0])
                                           *(B2_myx[2* n1   *(m+1)+p  ][0] - B2_myx[2* n1   * m   +p+1][0])
                                           )
    ...
    # writeexcel(T1_up,  'T1_up',  2)
    # writeexcel(T1_down,'T1_down',2)
    # writeexcel(N_B1,'N_B1',2)
    # writeexcel(N_B1_fanshu,'N_B1_fanshu',1)
    # writeexcel(N_B1_danwei,'N_B1_danwei',2)
    # writeexcel(B1_tou,'B1_tou',2)
    # writeexcel(B1_myx,'B1_myx',2)
    # writeexcel(Average_B1,'Average_B1',2)

    # writeexcel(T2_up,  'T2_up',  2)
    # writeexcel(T2_down,'T2_down',2)
    # writeexcel(N_B2,'N_B2',2)
    # writeexcel(N_B2_fanshu,'N_B2_fanshu',1)
    # writeexcel(N_B2_danwei,'N_B2_danwei',2)
    # writeexcel(B2_tou,'B2_tou',2)
    # writeexcel(B2_myx,'B2_myx',2)
    # writeexcel(Average_B2,'Average_B2',2)
    
    # writeexcel(A_up,'A_up',1)
    # writeexcel(A_down,'A_down',1)

    # writeexcel(T1_up_fanshu,'T1_up_fanshu',1)
    # writeexcel(T1_up_danwei,'T1_up_danwei',2)
    # writeexcel(T1_down_fanshu,'T1_down_fanshu',1)
    # writeexcel(T1_down_danwei,'T1_down_danwei',2)
    # writeexcel(m_danwei_up,'m_danwei_up',2)
    # writeexcel(m_danwei_down,'m_danwei_down',2)

    # writeexcel(ZhiXin_up,'ZhiXin_up',2)
    # writeexcel(Zhixin_up_fxqzbx,'Zhixin_up_fxqzbx',2)
    # writeexcel(ZhiXin_down,'ZhiXin_down',2)
    # writeexcel(Zhixin_down_fxqzbx,'Zhixin_down_fxqzbx',2)
...
#切楔/切锥法1-文献4
def Qie1(zhuangjijiao,Vx,Vy,Vz,Ma,*args,**kwargs):
    ...
    Cp=2.0*exp(e)*sin(zhuangjijiao)**2
    return Cp

#切楔/切锥法1-文献5
def Qie2(zhuangjijiao,Vx,Vy,Vz,Ma,*args,**kwargs):
    Cp=(4.0*sin(zhuangjijiao)**2*(2.5+8*sin(zhuangjijiao)*(sqrt(Ma**2-1)))
            /(
                1+16.0*sin(zhuangjijiao)*sqrt(Ma**2-1)
              )
        )
    return Cp
...
def writeexcel(data,name,dimension):
    try:
        wb=openpyxl.load_workbook(excelFileName)    #打开名为...的excel文件
    except:
        wb=openpyxl.Workbook()                              #如果打不开就创建一个
    try:                                                    #删除名为Sheet的表格
    ...
    wb.create_sheet(index=len(sheetnames)+1,title=name)     #创建一个sheet
    ...
    if dimension==1:
        ...
        i=0
        while i<row_num:
            sheet.cell(i+1,1).value=data[i]
            i+=1
    wb.save(excelFileName)                         #将创建的excel文档以此名称保存
