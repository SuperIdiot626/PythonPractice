...
nx = 51    # x grid points number
ny = 51    # x grid points number
nt = 1000     # time steps
c = 1       # wave speed
nu = 0.01   #viscousity
...
p[:, 0] = 0         #p=0 @ x=0
p[:,-1] = 0#y         #p=y @ x=1
p[ 0,:] = 0#p[ 1,:]   # dp/dy = 0 @ y = 0 因为最外边的一圈实际上未参与计算，所以要对外圈进行赋值
p[-1,:] = 0#p[-2,:]   # dp/dy = 0 @ y = 1

#b[int(1/4*ny),int(1/4*nx)] = -100
#b[int(3/4*ny),int(3/4*nx)] = -100
b[int(2/4*ny),int(2/4*nx)] = 200
b[int(1/4*ny),int(2/4*nx)] = -50
b[int(2/4*ny),int(1/4*nx)] = +50
b[int(3/4*ny),int(2/4*nx)] = -50
b[int(2/4*ny),int(3/4*nx)] = +50
